    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                pageNO: 1,
                //列表数据
                listurl: '<#if package.ModuleName??>/${package.ModuleName}</#if>/<#if controllerMappingHyphenStyle??>${controllerMappingHyphen}<#else>${table.entityPath}</#if>/listForPage',
                addurl: '<#if package.ModuleName??>/${package.ModuleName}</#if>/<#if controllerMappingHyphenStyle??>${controllerMappingHyphen}<#else>${table.entityPath}</#if>/add',
                editurl: '<#if package.ModuleName??>/${package.ModuleName}</#if>/<#if controllerMappingHyphenStyle??>${controllerMappingHyphen}<#else>${table.entityPath}</#if>/edit',
                deleteurl: '<#if package.ModuleName??>/${package.ModuleName}</#if>/<#if controllerMappingHyphenStyle??>${controllerMappingHyphen}<#else>${table.entityPath}</#if>/deleteBatch',
                loading: true,
                listData: [],
                multipleSelection: [],
                tableHeader: [
<#list table.fields as field>
{prop: '${field.propertyName}', label: '${field.comment}', minWidth: '100px' <#if field.keyIdentityFlag>,isHidden: true</#if>},
</#list>
        ...
        methods: {
            ...
            deleteManyRows(){
                ...
                this.$confirm(
                    '此操作将永久删除[' + this.multipleSelection[0].username + '等'+this.multipleSelection.length+'条记录], 是否继续?', '提示', {
                        confirmButtonText: '确定',
                        cancelButtonText: '取消',
                        type: 'warning'
                    }).then(() => {
