...
<div region="center" border="false" style="overflow: hidden;">
    <table id="dg" class="easyui-datagrid"
           url="${pageContext.request.contextPath}/roles/listForPage"
        ...
        <tr>
            ...
            <th field="role" width="100"></th>
            <th field="description" width="100"></th>
            <th field="available" width="100"></th>
        </tr>
        </thead>
    </table>
    <div id="toolbar">
    ...
    <div id="dlg" class="easyui-dialog"
        ...
        <form id="fm" method="post" novalidate style="margin:0;padding:20px 50px">
            <table>
                ...
                <tr>
                    <td>
                        <input name="role" class="easyui-textbox" label=":" style="width:100%">
                    </td>
                </tr>
                <tr>
                    <td>
                        <input name="description" class="easyui-textbox" label=":" style="width:100%">
                    </td>
                </tr>
                <tr>
                    <td>
                        <input name="available" class="easyui-textbox" label=":" style="width:100%">
                    </td>
                </tr>
            </table>
        </form>
...
//        $('#dg').datagrid('load',obj);
    ...
    function editObject() {
        ...
        if (row) {
            $('#dlg').dialog('open').dialog('center');
            $('#fm').form('load', row);
            url = __path + '/roles/edit?id=' + row.id;
        }
    }
    function saveObject() {
        $('#fm').form('submit', {
    ...
    function destroyObject() {
        ...
        if (row) {
            $.messager.confirm('Confirm', '你确定删除这条记录吗?', function (r) {
                if (r) {
                    $.post('/roles/delete', {ids: row.id}, function (result) {
                        if (result.result) {
                            $('#dg').datagrid('reload');    // reload the user data
                        } else {
