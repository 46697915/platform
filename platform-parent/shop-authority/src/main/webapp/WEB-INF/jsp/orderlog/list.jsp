...
<script type="text/javascript">
	...
	function addOrderlog(){
		...
		$('#fm').form('clear');
		url=path+"/orderlog/add";
		mesTitle = '新增成功';
	}
	
	//编辑用户信息
 	function editOrderlog(){
	 	var row = $('#datagrid').datagrid('getSelected');
	 	if (row){
	 		var id = row.id;
		 	$('#dlg').dialog('open').dialog('setTitle','编辑');
		 	$('#fm').form('load',row);
		 	url = path+"/orderlog/modify?id="+id;
		 	mesTitle = '编辑成功';
	 	}else{
	...
	//删除用信息
 	function deleteOrderlog(){
	 	var row = $('#datagrid').datagrid('getSelected');
	 	if (row){
	 		var id = row.id;
		 	$('#dlg_delete').dialog('open').dialog('setTitle','删除');
		 	$('#fm').form('load',row);
		 	url = path+"/orderlog/delete?id="+id;
		 	mesTitle = '删除成功';
	 	}else{
...
<body class="easyui-layout" fit="true">
	<div region="center" border="false" style="overflow: hidden;">
		...
		<table id="datagrid" class="easyui-datagrid" 
			...
			<thead>
				<tr>
					...
					<th field="operate" width="100">操作内容</th>
					<th field="operatetype" width="100">操作员类型</th>
		...
		<div id="toolbar">
			...
			<a href="javascript:void(0);" class="easyui-linkbutton"
				iconCls="icon-add" plain="true" onclick="addOrderlog();">新增</a> 
			<a href="javascript:void(0);" class="easyui-linkbutton"
				iconCls="icon-edit" plain="true" onclick="editOrderlog();">编辑</a> 
			<a href="javascript:void(0);" class="easyui-linkbutton"
				iconCls="icon-remove" plain="true" onclick="deleteOrderlog();">删除</a>
			<span>用户名:</span><input name="search_username" id="search_username" value="" size=10 /> 
  			<a href="javascript:searchOrderlogQ()" class="easyui-linkbutton" data-options="iconCls:'icon-search'">查询</a> 
			<a href="javascript:void(0);" class="easyui-linkbutton"
				iconCls="icon-jright" plain="true" onclick="searchOrderlog();">更多查询</a>
		</div>
		...
		<div id="dlg" class="easyui-dialog"
			...
			<form id="fm" method="post" novalidate>
				...
				<div class="fitem">
					<label>操作内容:</label> <input name="operate" class="easyui-textbox" required="true">
				</div>
				<div class="fitem">
					<label>操作员类型:</label> <input name="operatetype" class="easyui-textbox" required="true">
				</div>
		...
		<div id="dlg-buttons">
			<a href="javascript:void(0)" class="easyui-linkbutton c6"
				iconCls="icon-ok" onclick="saveOrderlog()" style="width:90px">保存</a> 
				<a href="javascript:void(0)" class="easyui-linkbutton"
		...
		<div id="dlg-del-buttons">
			<a href="javascript:void(0)" class="easyui-linkbutton c6"
				iconCls="icon-ok" onclick="saveOrderlog_del()" style="width:90px">删除</a> 
			<a href="javascript:void(0)" class="easyui-linkbutton"
		...
		<div id="dlg-buttons">
			<a href="javascript:void(0)" class="easyui-linkbutton c6"
				iconCls="icon-ok" onclick="searchOrderlog()" style="width:90px">查询</a> 
				<a href="javascript:void(0)" class="easyui-linkbutton"
