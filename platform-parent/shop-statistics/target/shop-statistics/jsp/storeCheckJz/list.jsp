...
<script type="text/javascript">
	...
	//编辑用户信息
 	function editstorecheckJz(){
	 	...
	 	if (row){
		 	...
		 	$('#fm').form('load',row);
		 	url = __path+"/storecheckJz/modify?id="+id;
		 	mesTitle = '编辑成功';
	 	}else{
	...
	//删除用信息
 	function deletestorecheckJz(){
	 	...
	 	if (row){
		 	...
		 	$('#fm').form('load',row);
		 	url = __path+"/storecheckJz/delete?id="+id;
		 	mesTitle = '删除成功';
	 	}else{
	...
	//快速查询
 	function searchstorecheckJzQ(){
		...
		options.url = __path + '/storecheckJz/listForPage';
	 	$("#datagrid").datagrid("load", {
	 		"barcode": $('#barcode_search').combobox('getValue'),
	 		"iserrorstore": $("input:checkbox:checked").val(),
            "crrcheckdate_begin": $('#crrcheckdate_begin').datebox('getValue'),
			"crrcheckdate_end": $('#crrcheckdate_end').datebox('getValue'),
	...
	function exportExcel(){
		...
		if(!isErrorStore){
 		...
 		var method = __path + '/storecheckJz/exportStoreCheckExcel'+
 			'?barcode='+$('#barcode_search').combobox('getValue')+
	...
	function cleanSearch(){
		...
		$('#crrcheckdate_begin').datebox('setValue','');
		$('#crrcheckdate_end').datebox('setValue','');
		$('#newstore_search').val('');
	}
	
	//盘点库存
	function storecheckJz(){
	...
	function deleteSCByDate(){
		...
		$.ajax({
			type : "POST",
			url  : __path+'/storecheckJz/deleteSCByDate',
			data : {
				crrcheckdate: crrcheckdate_search
			},
			success : function(result){
	...
	<div region="center" border="false" style="overflow: hidden;">
		...
		<div id="toolbar">
			...
			<a href="javascript:exportExcel()" class="easyui-linkbutton" data-options="iconCls:'icon-search'">导出</a> 
  			<%--<br>--%>
  			<%--<a href="javascript: storecheckJz()" class="easyui-linkbutton" data-options="iconCls:'icon-edit'">盘点库存</a>--%>
